...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setStatus({ loading: true, message: 'Initiating transmission...', error: false });
    ...
    try {
      ...
      if (data.success) {
        setStatus({ loading: false, message: 'Transmission Successful. Connection established.', error: false });
        ...
        confetti({
          ...
          spread: 70,
          origin: { y: 0.6 },
          colors: ['#00f2ff', '#3b82f6', '#10b981']
        });
      } else {
        setStatus({ loading: false, message: data.message || 'Transmission failed. Signal lost.', error: true });
      }
    } catch (error) {
      setStatus({ loading: false, message: 'Critical Error: Backend unreachable.', error: true });
    }
  };
          ...
          <SpotlightCard className="p-8 bg-zinc-900/40 border border-white/5 hover:border-cyan-400/50 group cursor-pointer transition-all">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-gray-400 font-mono text-[10px] sm:text-xs md:text-sm tracking-wide group-hover:text-cyan-400 transition-colors break-all">
             ...
             <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {status.message && (
                  <motion.div 
                    ...
                    className={`text-sm font-mono p-3 rounded-lg border ${status.error ? 'bg-red-500/10 border-red-500/50 text-red-400' : 'bg-cyan-500/10 border-cyan-500/50 text-cyan-400'}`}
                  >
                    {status.message}
                  </motion.div>
                )}
                ...
                <motion.button 
                  ...
                  className={`w-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all ${status.loading ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-[0_0_30px_rgba(6,182,212,0.5)]'}`}
                >
                  {status.loading ? 'Sending...' : 'Send Transmission'} <Send size={18} />
                </motion.button>
             </form>
          </SpotlightCard>
        </motion.div>
